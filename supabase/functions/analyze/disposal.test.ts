...
test("an official catalog match remains eligible", () => {
  const result = classifyDisposal(
    { label: "일반 의자", category: "furniture" },
    true,
  );
...
test("an official free-collection match explains the zero fee", () => {
  const result = classifyDisposal(
    { label: "전자레인지", category: "appliance_small" },
    true,
    true,
  );

  assert.equal(result.bulky_waste_status, "eligible");
  assert.match(result.disposal_notice ?? "", /무상수거/);
});
